...
- Give nontrivial executable scripts a `--show-plan-only` mode when they can
  ...
  require privileged or environment-specific live probes; it may inspect
  project-owned local markers when that improves the preview. It should say
  which checks and actions were intentionally not performed. Do not add a
  meaningless plan flag to a simple read-only validator that is not part of a
  plan-mode workflow.
