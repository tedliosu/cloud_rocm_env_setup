...
canonicalize_rocm_validation_root "${TEST_ROCM_LINK}" > "${TEST_REPORT}"
[ "$(<"${TEST_REPORT}")" = "${TEST_ROCM_ROOT}" ]
[ "${ROCM_HOME}" = "/ambient/rocm" ]
[ "${LD_LIBRARY_PATH}" = "/ambient/lib" ]

if canonicalize_rocm_validation_root >/dev/null 2>&1 ||
    canonicalize_rocm_validation_root one two >/dev/null 2>&1; then
...
for _invalid_result in "" "relative/rocm" $'/first/root\n/second/root' \
    ...
    if canonicalize_rocm_validation_root \
