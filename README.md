...
- [ ] Implement DevCloud early firewall initialization and existing-system upgrade:
    - `setup/amd_devcloud/bin/amd_devcloud_env_setup.sh` runs the accepted read-only ordinary-user handoff preflight, Ubuntu 24.04 identity check, finite AMDGPU/ROCm admission check, shared conservative UFW initializer, existing-package upgrade, and acknowledged system-upgrade reboot in that order.
