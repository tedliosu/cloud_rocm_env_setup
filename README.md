...
- [ ] Implement DevCloud ROCm 7.2.3 and the common minimum baseline:
    ...
    - Print and document command-scoped `ROCM_HOME=/opt/rocm-7.2.3` and `LD_LIBRARY_PATH=/opt/rocm-7.2.3/lib`; do not add an unobserved `lib64` path. Make the common validator derive the complete PATH-selected root from `hipconfig --rocmpath`, resolve and report its canonical directory, and scope those variables to the validator process rather than truncating a versioned path to or otherwise depending on the mutable `/opt/rocm` alternative. Do not edit `/etc/ld.so.conf`, add a project path under `/etc/ld.so.conf.d/`, run `ldconfig` merely to expose this stack globally, or persist ROCm runtime-selection variables in shell startup files.
