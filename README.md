...
- [ ] Implement DevCloud early firewall initialization and existing-system upgrade:
    - `setup/amd_devcloud/bin/amd_devcloud_env_setup.sh` runs the accepted read-only ordinary-user handoff preflight, Ubuntu 24.04 identity check, finite AMDGPU/ROCm admission check, shared conservative UFW initializer, existing-package upgrade, tmux installation, and acknowledged system-upgrade reboot in that order.
    ...
    - Refresh APT metadata, upgrade only the already-installed system packages, install tmux for persistent sessions during the longer post-reboot stages, record system-upgrade pending reboot state, and reboot.
...
- [ ] Implement the pinned DevCloud AMDGPU driver installation:
    ...
    - Install the verified local package through APT so its declared dependencies are resolved rather than assuming a fixed prerequisite list. Then refresh APT metadata and fail clearly if the newly configured AMD repositories are not usable.
    ...
    - Run this repository-bootstrap stage only after acknowledging the first reboot, and write its milestone only after the exact installed package, commands, paths, and usable APT repositories have been verified.
    ...
    - Install `amdgpu-dkms` Debian version `1:6.16.13.30300300-2327507.24.04` together with the headers and modules-extra package for the running kernel. Its packaged `dkms.conf` declares module version `6.16.13-2327507.24.04`; keep the Debian package and DKMS module versions distinct. Install versioned `amd-smi-lib7.2.3` version `26.2.2.70203-90~24.04`, rather than introducing the unversioned ROCm package line. These are the exact Noble package values observed in the pinned AMD 7.2.3 repositories on 2026-09-12 and confirmed from the downloaded driver package on 2026-09-13.
    - The versioned `rocm-core7.2.3` dependency maintains `/opt/rocm` through Debian alternatives. Recognize the driver-stage path only when that symlink resolves exactly to `/opt/rocm-7.2.3`; continue to use the explicit versioned root for setup and validation rather than depending on the mutable alternative.
    - After installing the matching AMDGPU DKMS and AMD SMI packages, require a driver-installation-specific acknowledged reboot.
    - After reboot, require the observed MI300X VF, native `gfx942`, successful DKMS state, and AMD SMI driver version `6.16.13`. The exact driver value and the alternatives target were observed after the first controlled DevCloud driver installation and reboot on 2026-09-13.
